...
export class VideoCallGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleOffer(@MessageBody() data: OfferAnswerDto, client: Socket) {
    console.log(`Offer received in room ${data.roomId}`);
    client.to(data.roomId).emit('offer', data.sdp);
  }

  @SubscribeMessage('answer')
  handleAnswer(@MessageBody() data: OfferAnswerDto, client: Socket) {
    client.to(data.roomId).emit('answer', data.sdp);
  }

  @SubscribeMessage('ice-candidate')
  handleIceCandidate(@MessageBody() data: IceCandidateDto, client: Socket) {
    client.to(data.roomId).emit('ice-candidate', data.candidate);
  }

  @SubscribeMessage('join')
  handleJoinRoom(client: Socket, @MessageBody() roomId: string) {
